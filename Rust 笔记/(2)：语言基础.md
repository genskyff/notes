...
在编译时就已知的大小的类型为**固定大小类型**，具有 `Sized` trait；在运行时才知道大小的类型为**动态大小类型**（Dynamically Sized Types，DST），具有 `?Sized` trait。如切片和 trait 对象都是 DST，这种类型都不能直接使用，需要通过一个指向它们的指针来间接使用。

-   指向 DST 的指针大小是固定的，但是为指向 `Sized` 的指针大小的两倍；
    -   指向切片的指针额外存储了切片的元素数量；
...
-   可以为 DST 实现 trait，与泛型类型参数不同，`Self: ?Sized` 是 trait 定义中的默认值；
...
`let` 语句通过不可反驳的模式引入了一组新的变量。除非被另一个变量声明隐藏，否则任何变量从声明点到封闭块作用域的结束都是可见的，
...
```rust
let s: &str = "hello";  // 实际上会被推导为 &'static str
...
结构体使用 `struct` 定义，没有任何字段的为**类单元结构体**，通常用于不存储数据仅实现某个 trait 时使用。没有字段名只有类型的为**元组结构体**。
