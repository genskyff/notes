...
裸指针可以是未对齐的或为 `null`。当使用 `*` 解引用裸指针时，必须对齐且非 `null`，因此对裸指针**进行解引用是不安全的**。
...
任何能够在编译期就知道大小的类型，都具有 `Copy`，在赋值时会自动进行复制。而在堆上的数据则不具有该 trait，但可能具有 `Clone`，可以显式调用 `clone` 函数来复制数据。

对于引用类型，所有的 `&T` 都具有 `Copy`，而 `&mut T` 则不具有。
