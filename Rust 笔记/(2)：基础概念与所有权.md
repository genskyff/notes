...
`x` 和 `y` 都是固定大小类型，直接存放在栈上，因此 `y` 获取 `x` 的拷贝。而对于 `String`，`s2` 并不会获取一个 `s1` 的拷贝。
...
>   更多关于 `Result` 的方法，可参考 [Option in std::option](https://rustwiki.org/zh-CN/std/option/enum.Option.html#implementations)。
...
这样的语句中变量名位于 `PATTERN` 位置，然后将 `EXPRESSION` 中的值解构后绑定到对应的变量。
...
**匹配守卫**是一个指定于 `match` 分支模式之后的额外 `if` 条件，它也必须被满足才能匹配该分支。
...
闭包也叫做 Lambda 表达式，其定义了一个闭包类型，每个闭包都具有唯一性和匿名性。可以在一个地方创建闭包，然后在不同的上下文中执行闭包运算。
...
闭包通常很短，只关联小范围的上下文，因此编译器可推断出参数和返回值的类型，可以不用像函数定义那样必须显式指明类型。
...
这三种捕获值的方式对应三个 `Fn` trait，所有函数都自动实现了这三个 trait，所有闭包都自动实现了其中一个 trait：
...
let mut add_suffix = || s.push_str("!");  // FnMut 必须加上 mut
...
此外，当返回一个闭包时，必须使用 `move` 关键字，因为在离开函数作用域时，任何通过引用捕获的值都被丢弃。
...
闭包实际上是通过一个特殊的结构体实现的。每个闭包都是一个结构体对象，其中包含了闭包的代码和从环境中捕获的变量。该结构体对象实现了一个或多个 `Fn` trait，以便可以像函数一样使用它。当定义一个闭包时，编译器会根据闭包的代码和捕获的变量生成一个结构体类型，该结构体类型实现了对应的 `Fn` trait。这也就是为什么 `FnMut` 闭包必须加上 `mut`，因为修改捕获的值相当于修改该结构体存储的变量。
...
`Result<T, E>` 枚举通常用于可恢复错误，其含有 `Ok` 和 `Err` 这两个成员：
...
`File::open` 返回的 `Err` 的类型为 `io::Error`，该结构体的 `kind` 方法返回 `io::ErrorKind` 枚举，该枚举的成员对应 IO 操作可能导致的不同错误类型。
...
>   更多关于 `Result` 的方法，可参考 [Result in std::result](https://rustwiki.org/zh-CN/std/result/enum.Result.html#implementations)。
