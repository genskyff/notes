...
```shell
rustc --crate-name mylib --edition=2021 src/main.rs --out-dir target/debug/deps -C incremental=target/debug/incremental -L dependency=target/debug/deps
```
...
```shell
rustdoc --crate-name mylib src/lib.rs -o target/doc -L dependency=target/debug/deps
```
