...
除了默认的四种 profile，还可以自定义。当定义 profile 时，必须指定 `inherits`，用于说明当配置缺失时，缺失项要基于哪个 profile。
...
若该包是一个库 Crate，则还可以再添加 `[features]` 来配置 features，以 `package/feature` 格式来指定，这样这个包的 features 也可以给其它包用。
...
-   **Errors**：若函数返回 `Result<T, E>`，此部分描述可能会出现的错误以及什么情况会造成这些错误；
