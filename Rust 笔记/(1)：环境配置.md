...
工作区是一系列包的集合，这些包被称为**工作区成员**，并被一起管理。一个包可以包含一个库 Crate，但随着内容持续增多，将其拆分为多个 Crate 更便于管理。

-   通用命令可以在所有工作区成员上运行，如 `cargo check --workspace`；
-   所有工作区成员共享工作区根目录下的 *Cargo.lock* 和 *target*；
-   共享包元数据，如 `workspace.package`。

### 创建

创建工作区只需要在工作区根目录创建一个 *Cargo.toml*，并添加 `[workspace.members]`，至少需要包含一个成员。
...
构建时 Cargo 并不假定工作区中的 Crate 之间会相互依赖，所以需要在各 Crate 的 *Cargo.toml* 中明确标明依赖关系。
...
这时可以使用 `cargo build` 来构建整个项目了。由于目前工作区只包含一个二进制 Crate，所以可以直接使用 `cargo run` 来运行，但若包含多个二进制 Crate，就需要使用 `-p` 参数来指定到底运行哪个二进制carte。
