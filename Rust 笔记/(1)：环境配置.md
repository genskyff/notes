...
在 *Cargo.toml* 中添加 `[features]` 字段以及在给依赖指定 `features`，可以给包添加条件编译和可选依赖的功能。

`[features]`特性在表中定义`Cargo.toml`。每个功能都指定一系列其他功能或它启用的可选依赖项。以下示例说明了如何将功能用于 2D 图像处理库，其中可以选择包含对不同图像格式的支持：
...
定义此功能后，`cfg`表达式可用于有条件地包含代码以在编译时支持所请求的功能。例如，`lib.rs`包内可能包含以下内容：
...
`rustc` `--cfg`Cargo 使用标志设置包中的功能，代码可以使用`cfg`属性或`cfg`宏测试它们的存在。
...
默认情况下，除非明确启用，否则所有功能均被禁用。这可以通过指定功能来更改`default`：
...
构建包后，`default`会启用该功能，从而启用列出的功能。可以通过以下方式更改此行为：

-   命令`--no-default-features` 行标志禁用包的默认功能。
-   该选项可以在依赖声明`default-features = false`中指定。

>   **注意**：选择默认功能集时要小心。默认功能很方便，可以更轻松地使用包，而无需强迫用户仔细选择启用哪些功能以供常用，但也有一些缺点。`default-features = false`除非指定，否则依赖项会自动启用默认功能。这可能会导致难以确保默认功能未启用，尤其是对于在依赖关系图中多次出现的依赖关系。每个包都必须确保`default-features = false`指定以避免启用它们

#### 可选依赖项

依赖项可以标记为“可选”，这意味着默认情况下不会编译它们。例如，假设我们的 2D 图像处理库使用外部包来处理 GIF 图像。这可以这样表达：
...
这意味着仅当`gif` 启用该功能时才会包含此依赖项。代码中可以使用相同的`cfg(feature = "gif")`语法，并且可以像任何功能一样启用依赖项，例如`--features gif`

在某些情况下，您可能不希望公开与可选依赖项同名的功能。例如，可选依赖项可能是内部细节，或者您想要将多个可选依赖项分组在一起，或者您只是想使用更好的名称。`dep:`如果您在表中的任何位置指定带有前缀的可选依赖项`[features]`，则会禁用隐式功能。

>   **注意**：该`dep:`语法仅从 Rust 1.60 开始可用。以前的版本只能使用隐式功能名称。

例如，假设为了支持 AVIF 图像格式，我们的库需要启用其他两个依赖项：
...
在此示例中，该`avif`功能将启用两个列出的依赖项。这也避免了创建隐式`ravif`和`rgb`功能，因为我们不希望用户单独启用它们，因为它们是我们板条箱的内部细节。

>   **注意**：选择性包含依赖项的另一种方法是使用特定于平台的依赖项。这些不是使用功能，而是基于目标平台的条件。

#### 依赖特性

可以在依赖项声明中启用依赖项的功能。键 `features`指示要启用哪些功能：
...
可以使用以下方法禁用 这些`default`功能`default-features = false`：
...
>   **注意**：这可能无法确保禁用默认功能。如果另一个依赖项包含`flate2`但未指定`default-features = false`，则将启用默认功能。有关更多详细信息，请参阅下面的功能统一。

还可以在表中启用依赖项的功能`[features]`。语法是`"package-name/feature-name"`. 例如：
...
如果它是可选依赖项，该`"package-name/feature-name"`语法也将启用。`package-name`通常这不是您想要的。您可以添加一个`?`as，其中`"package-name?/feature-name"`只有在其他功能启用可选依赖项时才会启用给定功能。

>   **注意**：该`?`语法仅从 Rust 1.60 开始可用。
...
在此示例中，启用该`serde`功能将启用 serde 依赖项。它还将为依赖项启用该`serde`功能`rgb`，但前提是其他东西已启用该`rgb`依赖项。
