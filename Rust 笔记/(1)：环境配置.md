...
*~/.rustup/settings.toml* 保存了工具链相关的配置。*~/.cargo/config.toml* 保存了 Cargo 构建相关的配置，且会覆盖项目目录中 *Cargo.toml* 的配置，但通过命令行参数直接传递配置则会覆盖以上两者。配置文件使用 [TOML](https://toml.io/cn/v1.0.0) 格式。
...
在 *~/.cargo/config.toml* 和项目目录的 *Cargo.toml* 中可以配置构建相关参数来影响编译器行为。

### profile 配置

`profile` 为发布配置，默认包含四种：

-   **dev**：`cargo build` 使用，用于开发；
-   **release**：`cargo build -r` 使用，用于发布；
...
然后在构建时使用 `--profile` 选项来指定自定义 profile：
