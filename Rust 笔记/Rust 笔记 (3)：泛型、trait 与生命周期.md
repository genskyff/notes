...
使用 `<T, U, ..>` 语法来定义泛型，并可用于结构体、枚举、联合体、函数、实现、和 trait。其中的 `T, U, ..` 为泛型参数，可以看作是占位符，在编译阶段会被实际的类型所替代。
...
其顺序被规定为：生命周期参数必须放置于后两类之前，后两类可以交叉摆放。其中类型参数最为常用，而常量参数通常是具体类型，目前仅允许除了浮点型以外的基本类型作为常量参数。
