...
生命周期参数是为了将函数参数和返回值进行关联，标注后编译器就有了足够的信息检查并阻止如悬垂引用这类违反内存安全的行为。

如 `ret_x` 只会返回 `x`，因此返回值仅与 `x` 有关联，因此这里 `y` 的生命周期参数就无关紧要。
...
若返回的引用没有与任何参数关联，那么可能是指向函数内部创建的值。
...
对于 `ret_str` 即使手动标注了生命周期，也无法通过编译，因为 `s` 所指向的堆内存在函数返回后就被回收了，这会造成悬垂引用。生命周期参数只是用来给编译器检查实际传递的值是否符合规则的标注，无法改变生命周期。
