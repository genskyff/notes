...
**生命周期用于描述引用保持有效的作用域。**大部分情况生命周期是隐含并可以推断的，但也有需要显式标注生命周期参数。
...
作用域越大，生命周期就越长。Rust 中有一个**借用检查器**，它比较作用域来确保所有的借用都是有效的。
...
这里将 `outer` 的生命周期标记为 `'a`，将 `inner` 的生命周期标记为 `'b`，显然 `'b` 比 `'a` 小。编译时，编译器比较这两个生命周期的大小，并发现 `outer` 拥有生命周期 `'a`，但借用了一个拥有生命周期 `'b` 的对象。由于 `inner` 比 `outer` 存在的时间更短，因此在使用 `outer` 时，其引用可能会失效。
