...
**生命周期用于描述引用保持有效的作用域**。大部分情况下生命周期是隐含并可以推断的，否则需要显式标注生命周期参数。

## 生命周期作用

生命周期的主要作用是避免悬垂引用。外部变量 `outer` 借用了内部变量 `inner`，但内部作用域结束后，`inner` 的内存会被释放，此时再使用 `outer` 就会造成悬垂引用。
...
作用域越大，生命周期就越长。Rust 中有一个**借用检查器**，用于比较作用域来确保所有的借用都是有效的。
...
这里将 `outer` 和 `inner` 的生命周期分别标记为 `'a` 和 `'b`，显然 `'b` 小于 `'a`。编译器会比较这两个生命周期的大小，并发现 `outer` 拥有生命周期 `'a`，但借用了一个拥有生命周期 `'b` 的对象。由于 `inner` 比 `outer` 存在的时间更短，因此在使用 `outer` 时，其引用可能会失效。
...
这里 `x` 的生命周期 `'a` 比 `y` 的生命周期 `'b` 要大，因此当 `x` 有效时，`y` 中的引用也肯定有效。
...
生命周期参数描述了多个引用之间生命周期的相互关系，**而不影响其生命周期**，仅用于让借用检查器分析。
...
这个字符串的文本被直接储存在程序的二进制文件的只读区块中，因此所有的字符串字面值都是 `'static` 的。
