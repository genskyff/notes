...
`longest` 函数接收两个引用作为参数，并返回其中一个。但这样无法通过编译，因为编译器并不知道将要返回的引用是来自 `x` 还是 `y`，也无法推断出参数与返回值之间生命周期的关系，因此无法确定返回的引用是否有效。
...
由于生命周期参数是用于描述多个引用之间的关系，因此对单个引用进行显式标注没有什么实际意义。对于 `longest` 函数，进行显式标注后则可以进行编译。
...
较长的生命周期可以转成一个较短的生命周期，转换可由编译器隐式推断，也可显式标注。实际上就是给生命周期参数加上了另一个生命周期参数作为约束。如 `<'a: 'b, 'b>` 表示 `'a` 的至少和 `'b` 一样长。
