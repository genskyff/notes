...
`String` 和 `Vec` 就是智能指针，因为它们拥有数据的所有权，同时也含有元数据（裸指针、长度和容量等）和额外的功能（如 `String` 含有各种方法，并保证数据总是有效的 UTF-8 编码）。
...
`Rc` 实现了 `Clone`，但只会在栈上复制 `Rc` 并增加引用计数，不会进行堆上的复制。每次调用 `Rc::clone`，引用计数都会加 1，而 `Rc` 离开作用域后会减 1，在减少到 0 之前堆数据都不会被清理。在实例上调用 `clone` 也是同样的效果，为了避免混淆，习惯使用 `Rc::clone`。

引用计数分两类：**强引用**和**弱引用**。`Rc::clone` 
