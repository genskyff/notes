...
所有配置文件的键值会被合并，重复的以优先级高的为准，相同优先级以最后定义的为准。命令行直接传递的优先级最高，项目中嵌套越深的优先级越高。执行 Cargo 时，当前目录的子目录中的配置文件会被忽略。

> - 配置文件使用 [TOML](https://toml.io/en/v1.1.0) 格式
...
默认情况下，所有 profile 都不会应用到构建脚本、过程宏和这两者的依赖，且不会显示调试信息，但可以配置 `profile.<name>.build-override` 来覆盖默认行为。
...
默认情况下，profile 会应用到所有包，但可以通过 `profile.<name>.package.<name>` 来指定特定包构建时所使用的 profile。
...
- `cargo::error=MESSAGE`：显示错误信息
- `cargo::warning=MESSAGE`：显示警告信息
- `cargo::metadata=KEY=VALUE`：元数据，由脚本使用 `links`
...
_Cargo.toml_ 的 `[package.links]` 用于声明需要链接的本地库，这是为了让 Cargo 收集本地依赖项，同时提供一个在包构建脚本之间传递元数据的系统。
...
当使用 `links` 时，项目必须有一个构建脚本，且该脚本需使用 `rustc-link-lib` 指令来链接目标库。
...
构建脚本无法使用 `[dependencies]` 或 `[dev-dependencies]` 中的依赖，因为构建脚本和编译过程是相互独立的，包本身也无法使用 `[build-dependencies]` 中的依赖。
