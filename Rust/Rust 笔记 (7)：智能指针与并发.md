...
**引用**也是一种指针，但只借用数据，没有数据的所有权，是裸指针的一种抽象，除了能解引用外没有其他功能，因此使用引用是安全的。
...
`Mutex` 允许线程等待直到解锁，但不提供检查其他条件的功能。要判断队列中是否为空，则当前线程必须不断检查队列是否为空。
...
当队列为空的时候，可以使用 `park` 让当前线程进行阻塞（睡眠），然后其他线程在将数据准备好后使用 `unpark` 唤醒线程。
...
- `unbounded`：队列没有上限，若写满了就自动扩容。和 `bounded` 相比，除了不阻塞，其他实现都很类似。
...
- `try_recv` 不会阻塞。其立刻返回一个 `Result`，`Ok` 包含接收到的值，`Err` 代表没有任何消息。若线程在等待消息过程中还有其他代码需要执行时使用 `try_recv`，如通过循环调用 `try_recv`，在有可用消息时进行处理，否则继续其他任务。
...
`LazyLock` 就是线程安全的 `LazyCell`。由于多个线程都调用初始化，因此若当前其他线程正在进行初始化，则任何解引用调用都将阻塞调用线程。
...
>   Relaxed 虽然最宽松，但依旧保证了原子操作本身的线程安全（不可分割、无数据竞争），并提供对同一内存位置操作的顺序保证，仅不提供跨内存位置操作顺序的保证。
