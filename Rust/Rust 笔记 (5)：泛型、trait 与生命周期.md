...
只有 `Point<f64>` 实现了 `distance`，而其他不是 `f64` 的 `T` 则没有实现此方法，但这种特化实现的方法名不能与其他实现相同。
...
默认实现可以调用 trait 中的其他方法，哪怕这些方法没有默认实现，因此可以通过这种方式来间接访问 `self` 中的字段。
...
因为 `Animal::name` 没有 `self` 参数，可能会有其他类型也实现了 `Animal`，因此编译器无法知道是哪个实现。
...
对于关联函数，其没有 `receiver`，因此只会有其他参数。可以选择在任何函数或方法调用处使用完全限定语法，但可省略任何编译器能够从程序的其他信息中计算出的部分，只有当存在多个同名实现时才使用这个较为冗长的语法。
...
`plus` 接受一个 `&mut i32`，因此 `p` 作为参数时会发生移动，从而导致第二次调用时出错，但实际上并没有，这是因为**再借用**机制。
...
这是因为 `borrow_forever` 发生了**永久借用**。当看到像 `&'a mut T<'a>` 这种声明，就代表是一个永久借用，因为这代表永远借用自己：被借用的对象生命周期为 `'a`，而借用也同样为 `'a`。因此该可变借用在 `T<'a>` 的生命周期中一直存在。根据借用规则，存在一个可变借用，自然就不再允许其他借用了。
