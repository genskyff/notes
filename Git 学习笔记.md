...
>   使用 `--list` 选项时，会查找所有级别的配置，并以优先级最高的配置为准，相同优先级若出现同名配置，则以最后找到的为准。
...
`restore`、`reset` 和 `revert` 都可以用来更改状态，但在功能上有一些区别。

### restore

1.  **用途**: 用于撤销工作目录和/或暂存区的更改。

2.  **安全性**: 相对安全，因为它不修改提交历史。

3.  **影响**: 只影响工作目录和暂存区，不影响提交历史。

4.  常见用法

    :

    -   撤销工作目录的更改：`git restore <file>`
    -   撤销暂存区的更改并恢复到最后一次提交：`git restore --staged <file>`

### reset

1.  **用途**: 主要用于重置 `HEAD` 指针，撤销提交，并可选地更改暂存区或工作目录。

2.  **安全性**: 可能不安全，因为它可以修改提交历史（如果用 `--hard` 选项）。

3.  **影响**: 可以影响提交历史、暂存区和工作目录。

4.  常见用法

    :

    -   软重置（不影响工作目录和暂存区）：`git reset --soft <commit>`
    -   混合重置（影响暂存区，但不影响工作目录，这是默认选项）：`git reset --mixed <commit>` 或 `git reset <commit>`
    -   硬重置（影响工作目录和暂存区）：`git reset --hard <commit>`

### revert

1.  **用途**: 用于创建一个新的提交，以撤销一个或多个旧的提交。

2.  **安全性**: 相对安全，因为它不修改现有的提交历史，而是添加新的提交。
...
-   **如果你想撤销工作目录或暂存区的更改，但不想影响提交历史**：使用 `git restore`。
-   **如果你想更改提交历史，并且/或重置暂存区和工作目录**：使用 `git reset`。
-   **如果你想撤销一个或多个提交，而不改变已有的提交历史**：使用 `git revert`。



`HEAD` 表示当前版本，上个版本是 `HEAD^`，上上个版本是 `HEAD^^`，往上 100 个版本写作 `HEAD~100`。
