...
这似乎有点反直觉，在目前的实现中，一个块就是一个作用域，且该作用域会随着执行而变化。当函数被声明时，会捕获当前环境的作为闭包，但该环境是可变的，因此闭包也就变得可变了。闭包应该捕获函数声明时的环境快照，且不会随着时间而改变。
...
而其他节点则不用额外处理，当然也可以做更多的检查，如检查 `break` 或 `return` 的位置是否正确，因为只能在相关上下文中使用，但这里也实现相关的访问者方法。
