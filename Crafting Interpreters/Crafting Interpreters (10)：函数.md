...
解析参数的数量没有限制，但大多数语言通常都对参数数量做了限制，如 Java 规定不超过 255 个参数，C 则要求至少要支持 127 个以上的参数。对于 Lox 来说，虽然可以没有限制，但设置最大参数数量可以简化后续字节码解释器的实现。
...
然后定义 `call` 方法，这部分是最重要的，因为参数是函数的核心，且参数只能在函数中使用，其他地方是无法看到的。这表示每个函数都有自己的环境，并在其中存储这些变量，且该环境必须动态创建，这样在递归调用时就不会影响调用者的环境，即使是同一个函数。
...
返回值是可选的，用以支持从不返回值的函数中提前返回，这相当于返回了 `nil`。
