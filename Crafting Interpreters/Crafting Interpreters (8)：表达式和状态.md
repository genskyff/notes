...
Lox 语法中并没有地方既允许表达式也允许语句，因此两种表达式和语句式不相干的。
...
变量声明是一种语句，但不同于其他语句，需要特殊处理 `stmt` 语法，因为语法需要限制某个位置上哪种类型的语句是被允许的。
...
或许也可以允许后者，但是这会导致 `s` 的作用域不清晰。语句好像有两种优先级，有些允许语句的地方，如程序顶层或块中，可以允许任何语句，其他地方只允许非声明语句和优先级更高的语句。
...
为了访问变量，需要对表达式中的 `primary` 产生式进行修改：
...
目前只有变量是有效的左值，其他语言中的对象字段访问、数组的索引访问等都是有效的左值。
...
当访问块中的每个语句时，跟踪所有变量，执行完块中最后一条一句时，删除环境中的所有变量，但要注意，只能删除该块所属的环境，否则其他作用域的环境会被干扰。
...
-   为现有变量赋值时，由于拼写错误，隐式创建了一个新变量而没有报错，这可能会干扰其他代码
