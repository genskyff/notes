...
虚拟机可以选择 [VirtualBox](https://www.virtualbox.org/wiki/Downloads) 或 [VMware Workstation](https://www.vmware.com/products/workstation-pro/workstation-pro-evaluation.html)，这两者使用方法都差不多，前者免费开源，后者功能强大并对个人免费。
...
- [下载地址](https://github.com/ExeinfoASL/ASL)
...
- [下载地址](https://tool.kanxue.com/index-detail-1.htm)
...
本文以 `VMnet1` 作为虚拟网卡，可以看到类型为 `仅主机`，以 `192.168.200.0/8` 作为虚拟网段，当然也可以设置为其他网卡和网段。
...
> 由于 Windows 默认安全设置，Ping 不通 Windows 是正常的。
...
> 一般不用修改 Windows 防火墙设置，这里只是演示这样做可以使其被 Ping。
...
默认情况下，INetSim 仅监听 `127.0.0.1`，为了使其能够监听虚拟网络中的其他机器，需要更改配置。
