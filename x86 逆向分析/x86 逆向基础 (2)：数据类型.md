...
全局变量还能够以外部变量的方式被其他文件访问，将上述 C 代码改成多文件的形式，如下：
...
全局变量可以在整个程序运行周期中一直存在，且可以被所有范围的代码访问，因此有较大风险。静态变量虽然和全局变量一样可以在整个程序运行周期中存在，但只能在定义的区域内访问。就算是在同一个文件中定义的全局变量，如果加上了 static 关键字，则不能被其他文件访问，即使加上 extern 关键字，编译后仍会报错，如下：
...
最高位 $x_{n-1}$ 为符号位，其权值为 $-2^{n-1}$，当最高位为 1 时，表示负数，否则表示正数。
...
反码除了最高有效位的权值为 $-(2^{n-1} - 1)$ 外，其余与补码相同：
...
- 阶码字段的位不全为 0 或 1，则表示一个规格化的值，在这种情况下 $E = e - Bias$，其中 $e$ 是以 n 位阶码字段表示的无符号整数，$Bias$ 是一个值为 $2^{n-1}$ 的偏置值，尾数字段的范围是 1 ≤ M < 2。可以通过调整阶码，来调整浮点数的精度。
