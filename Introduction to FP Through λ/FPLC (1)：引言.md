...
-   求值顺序无关性：对于一个函数调用 `f(a(x), b(x), c(x))`，由于函数 a、b、c 无法改变它们的共同参数 x，所以这三个函数调用的执行顺序不影响最终结果。这种特性是函数式语言的一大优势，为并行计算等优化提供了理论可能性。
...
-   λ 演算的应用：
    -   Landin：使用 λ 演算构建了 SECD 抽象机 来解释 Algol 60，为函数式语言的实现提供了模型
