...
**重复**（Repetition）：相同操作执行零次或多次

- **有界重复**（Bounded repetition）：重复次数已知，适用于处理已知长度的线性序列（如数组）
- **无界迭代**（Unbounded iteration）：直到满足条件，次数未知，适用于处理未知深度的嵌套结构（如文件系统）
