...
`rerere` （Reuse Recorded Resolution）可以让 Git 记住冲突是如何被解决的，当下次出现同样的冲突时，Git 会自动应用之前的解决方案。

使用场景：

-   长期变基：当在分支上开发时，若需要定期对主分支进行变基，则每次都要反复解决同一个冲突。开启 `rerere` 后，只需要手动解决一次，之后相同的冲突会被 Git 自动处理。
-   误合并后的恢复：当合并了一个分支，解决了复杂的冲突，结果发现合错了，于是撤销了。再次尝试合并时，Git 也会自动处理。

`rerere` 默认是关闭的，需要手动开启：
...
1.   记录：当遇到冲突并手动解决时，Git 会在后台记录冲突前的样子（上下文）和解决后的样子。提交后该记录会存入 `.git/rr-cache`。
2.   重用 ：当再次遇到相同的冲突时，Git 会自动应用保存的记录。
3.   检查与提交：Git 会自动应用，但为了安全不会 `add` 和 `commit`，需要手动检查并提交。
