...
在一阶逻辑中合式公式又称为**谓词公式**，简称**公式**，最外层括号可以省去。

>   **定义 2.5**
>
>   在合式公式 $\forall x A$ 和 $\exist x A$ 中，$x$ 为**指导变量**，A 为相应量词的**辖域**。在辖域中，$x$ 的所有出现称为**约束出现**，A 中不是约束出现的其它变项的出现称为**自由出现**。
...
在命题逻辑中，可以列出真值表，但是在一阶逻辑中，由于引入了函数和谓词，要进行类似的讨论，需要给公式中出现的所有个体常项符号、函数变项符号和谓词变项符号赋值，这就是**解释**。
...
>
>   1.   非空个体域 $D$
>   2.   给论及的所有个体常项符号指定一个 $D$ 中的元素
>   3.   给论及的所有函数变项符号指定一个 $D$ 上的函数
>   4.   给论及的所有谓词变项符号指定一个 $D$ 上的谓词

在使用解释 $I$ 解释公式 A 时，采用指定个体域 $D$，并将 A 中的所有个体常项符号、函数变项符号及谓词变项符号分别替换成 $I$ 中指定元素、函数及谓词。
...
>   1.   $D = \{ 2, 3 \}$
>   2.   $a = 2$
>   3.   函数 $f(x): f(2) =3, f(3) = 2$
>   4.   谓词 $F(x): F(2) = 0, F(3) = 1 \\ G(x, y): G(i, j) = 1, i, j = 2, 3 \\ L(x, y): L(2, 2) = L(3, 3) = 1, L(2, 3) = L(3, 2) = 0$​
>
>   在解释 $I$ 下，求下列各式真值：
>
>   1.   A：$\forall x (F(x) \and G(x, a))$
>   2.   B：$\forall x \exist y L(x, y)$
>
>   $$
>   A \Leftrightarrow (F(2) \and G(2, 2)) \and (F(3) \and G(3, 2)) \\
>   \Leftrightarrow (0 \and 1) \and (1 \and 1) \\
>   \Leftrightarrow 0 \\
>   B \Leftrightarrow (L(2, 2) \or L(2, 3)) \and (L(3, 2) \or L(3, 3)) \\
>   \Leftrightarrow (1 \or 0) \and (0 \or 1) \\
>   \Leftrightarrow 1
>   $$
...
1.   $D \in N$
2.   $a = 0$
3.   $f(x, y) = x + y, g(x, y) = x \cdot y$
4.   $F(x, y): x = y$

在解释 $N$ 下：

1.   $\forall x F(g(x, y), x) \Leftrightarrow \forall x (x \cdot 0 = x)$ 为假命题
2.   $\forall x \forall y \exist z F(f(x, y), z) \Leftrightarrow \forall x \forall y \exist z (x + y = z)$ 为真命题
3.   $F(f(x, y), f(y, z)) \Leftrightarrow x + y = y + z$ 的真值不确定，所以不是命题

可以看出，在给定的解释下，有的公式真值确定；是一个命题，有的不确定，不是命题。而对于闭式来说，由于每个个体变项都受到量词的约束，因此在任何解释下都是一个意义确定的语句——命题。对于非闭式公式，如果进一步给出每个自由出现的个体变项指定个体域中的一个元素，那么也成为一个命题。

给定解释 $I$，对公式中每个自由出现的个体变项指定个体域中的一个元素称作在解释 $I$​ 下的**赋值**。

>   **定义 2.8**
>
>   设 A 为谓词公式，若 A 在任何解释和该解释下的任何赋值都为真，则称 A 为**逻辑有效式**；若 A 在任何解释和该解释下的任何赋值下都为假，则称 A 是**矛盾式**；若至少存在一个解释和该解释下的一个赋值使 A 为真，则称 A 是**可满足式**。

从定义可知，逻辑有效式是可满足式，反之则不一定。

与命题公式不同，由于公式的复杂性和解释的多样性，谓词公式的可满足性是不可判定的，即不存在一个可行的算法，能够判断任一公式是否是可满足的，但某些特殊情况下还是可判断。

>   **定义 2.9**
>
>   设 $A_0$ 是含命题变项 $p_1, p_2, \cdots, p_n$ 的命题公式，$A_1, A_2, \cdots, A_n$ 是 $n$ 个谓词公式，用 $A_i$ 处处代换 $p_i (1 \le i \le n)$，所得公式 A 称为 $A_0$ 的**代换实例**。

如 $F(x) \rightarrow G(x), \forall x F(x) \rightarrow \exist x G(x)$ 都是 $p \rightarrow q$​ 的代换实例。

命题公式中的重言式的代换实例都是逻辑有效式，所有矛盾式的代换实例也同样是矛盾式。

>   **例**
>
>   1.   $\forall x F(x) \rightarrow \exist x F(x)$ 是一个闭式，只需要考虑解释，可看出是逻辑有效的。
>   2.   $\forall x F(x) \rightarrow (\forall x \exist y G(x ,y) \rightarrow \forall x F(x))$ 是 $p \rightarrow (q \rightarrow p)$ 的代换实例，该命题公式是重言式，所以是逻辑有效的。
>   3.   $\forall x F(x, y) \or \forall y F(x, y)$ 中第一个辖域中的 $y$ 和第二个辖域中的 $x$ 都是自由出现。取解释 $I$ 为个体域为自然数集 $N$，$F(x, y): x \le y$，取赋值 $\sigma_1: \sigma_1(x) = 0, \sigma_1(y) = 1$，在该解释和赋值下，公式为 $\forall x (x \le 1) \or \forall y (0 \le y)$，其值为真；再取赋值 $\sigma_2: \sigma_2(x) = 1, \sigma_2(y) = 0$，在该解释和赋值下，公式为 $\forall x (x \le 0) \or \forall y (1 \le y)$，其值为假。故该公式为非逻辑有效式的可满足式。

## 2.3 一阶逻辑等值式与前述范式

>   **定义 2.10**
>
>   设 A、B 是一阶逻辑中的两公式，若 $A \leftrightarrow B$ 为逻辑有效式，则称 A 与 B 是**等值的**，记作 $A \Leftrightarrow B$，称 $A \Leftrightarrow B$ 为**等值式**。

由于重言式都是逻辑有效式，因此命题公式中的等值式公式及其代换实例，都是一阶逻辑中的等值式。如：
$$
\forall x A(x) \Leftrightarrow \forall x A(x) \and \forall x A(x) \\
\forall x A(x) \rightarrow \exist x B(x) \Leftrightarrow \neg \forall x A(x) \or \exist x B(x)
$$
...
>   3.   $\forall x (A(x) \rightarrow B) \Leftrightarrow \exist x A(x) \rightarrow B$
...
>   3.   $\exist x (A(x) \rightarrow B) \Leftrightarrow \forall x A(x) \rightarrow B$
>   4.   $\exist x (B \rightarrow A(x)) \Leftrightarrow B \rightarrow \exist xA(x)$
>
>   其中 $A(x)$ 是含 x 自由出现的任意公式，而 B 中不含有 x 的自由出现。

证：$\exist x (A(x) \rightarrow B) \Leftrightarrow \forall x A(x) \rightarrow B$
$$
\exist x (A(x) \rightarrow B) \\
\Leftrightarrow \exist x (\neg A(x) \or B) \\
\Leftrightarrow \exist x \neg A(x) \or B \\
\Leftrightarrow \neg \forall x A(x) \or B \\
\Leftrightarrow \forall x A(x) \rightarrow B
$$
...
>   量词分配等值式：
>
>   1.   $\forall x (A(x) \and B(x)) \Leftrightarrow \forall x A(x) \and \forall x B(x)$
>   2.   $\exist x (A(x) \and B(x)) \Leftrightarrow \exist x A(x) \and \exist x B(x)$

>   **定理 2.4**
>
>   1.   $\forall x \forall y A(x, y) \Leftrightarrow \forall y \forall x A(x, y)$
>   2.   $\exist x \exist y A(x, y) \Leftrightarrow \exist y \exist x A(x, y)$

与命题逻辑的范式类似，一阶逻辑也存在谓词公式的规范形式。

>   **定义 2.11**
>
>   设 A 为谓词公式，若 A 具有以下形式：
>   $$
>   Q_1x_1Q_2x_2 \cdots Q_kx_kB
>   $$
>   则称 A 是**前束范式**，其中每个 $Q_i (i \le i \le k)$ 为 $\forall$ 或 $\exist$，B 为不含量词的谓词公式。

如 $\forall x \exist y (F(x, y) \rightarrow G(x, y))$、$\exist x \forall y \forall z (F(x, y, z) \rightarrow G(x, y, t))$ 都是前束范式，而 $\forall x F(x) \and \forall x G(x, y)$、$\forall x (F(x) \rightarrow \forall y (G(y) \rightarrow H(x)))$​ 都不是前束范式。

任何一阶逻辑的合式公式都存在与其等值的前束范式，可以通过定理以及换名规则得到。
...
> $$
> \forall x F(x) \and \neg \exist x G(x) \\
> \Leftrightarrow \forall x F(x) \and \forall x \neg G(x) \\
> \Leftrightarrow \forall x (F(x) \and \neg G(x))
> $$
>
> 2. 
> $$
> \exist x F(x) \rightarrow \forall x G(x) \\
> \Leftrightarrow \exist x F(x) \rightarrow \forall y G(y) \\
> \Leftrightarrow \forall x (F(x) \rightarrow \forall x G(x)) \\
> \Leftrightarrow \forall x \forall y (F(x) \rightarrow G(y))
> $$

**前述范式不是唯一的**，且各指导变项应该是不相同的。原公式的自由出现在前述范式中应该还是自由出现。若发现前束范式存在相同的指导变项，或自由出现变成约束出现，则说明换名规则使用有误。
