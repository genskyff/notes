...
设 R 是 A 上的关系，其性质有以下几种：

|              | 自反性                          | 反自反性                           | 对称性                                   | 反对称性                                                | 传递性                                                       |
| ------------ | ------------------------------- | ---------------------------------- | ---------------------------------------- | ------------------------------------------------------- | ------------------------------------------------------------ |
| 定义         | $\forall x \in A, <x, x> \in R$ | $\forall x \in A, <x, x> \notin R$ | $<x, y> \in R \rightarrow <y, x> \in R$  | $<x, y> \in R \and x \ne y \rightarrow <y, x> \notin R$ | $<x, y> \in R \and <y, z> \in R \rightarrow <x, z> \in R$    |
| 关系矩阵特点 | 主对角线元素为 1                | 主对角线元素为 0                   | 对称矩阵                                 | $r_{ij} = 1 \and i \ne j \rightarrow r_{ij} = 0$        |                                                              |
| 关系图特点   | 每个顶点都有环                  | 每个顶点都没环                     | 若两个顶点之间有边，则为一对方向相反的边 | 若两个顶点之间有边，则为一条有向边                      | 若顶点 $x_i$ 到 $x_j$ 有边，$x_j$ 到 $x_k$ 有边，则 $x_i$ 到 $x_k$ 有边 |

不难看出，A 上的 $E_A$ 和 $I_A$ 是自反的、对称的和传递的。整除关系、小于等于关系和幂集上的包含关系是自反的、反对称的和传递的。A 上的关系可以是自反的、反自反的，或既不是自反的，也不是反自反的。

设 $R_1, R_2$ 是 A 上的关系，具有某些性质，在经过并、交、补、逆、合成等运算后得到的新的关系所具有的性质为：
...
设 R 是非空集合 A 上的关系，有时需要 R 具有一些性质，如自反性、对称性或传递性。为此需要在 R 中添加一些有序对从而构成新的关系 $R'$，使得 $R'$ 具有所需要的性质，但又不需要 $R'$ 变得太大，即添加最小数量的有序对，使其具有所希望的性质，满足这些要求的 $R'$​ 就是 R 的**自反**（**对称**或**传递**）**闭包**。

>   **定义 4.11**
>
>   设 R 是非空集合 A 上的关系，R 的自反（对称或传递）闭包是 A 上的关系 $R'$，且 $R'$ 满足以下条件：
>
>   1.   $R'$ 是自反的（对称或传递的）
>   2.   $R \subseteq R'$
>   3.   对 A 上的任何包含 R 的自反关系（对称或传递关系）$R''$ 都有 $R' \subseteq R''$

一般将 R 的自反闭包记作 $r(R)$，对称闭包记作 $s(R)$，传递闭包记作 $t(R)$。

设 $A = \{a,b,c,d\}, R = \{<a,b>, <b,a>, <b,c>, <c,d>\}$

可以看出，R 不满足自反性、对称性和传递性。

要满足自反性，需要添加 $\{<a,a>, <b,c>, <c,c>, <d,d>\}$​

要满足对称性，需要添加 $\{<c,b>, <d,c>\}$

要满足传递性，需要添加 $\{<a,a>, <a,c>, <a,d>, <b,b>, <b,d>\}$
