...
**有类型语言**（Typed language）通过给程序变量规定明确的取值范围来避免错误。如一个类型为 Boolean 的变量 $x$ 在每次运行时只能取布尔值，使得表达式 $not(x)$ 始终有明确意义。而**无类型语言**（Untyped language）不限制变量的取值，或视所有值为单一通用类型（Universal type），这可能导致操作被应用于不适当的参数，从而产生任意结果、故障或异常。

-   一个极端例子是纯 λ 演算（Pure λ-calculus）：所有值都是函数，唯一的操作（函数应用）永远不会失败。

有类型语言可以通过在编译时进行静态检查从而强制程序具有良好行为，称为静态检查语言。这种检查过程称为**类型检查**（Typechecking），而执行这一检查的算法称为**类型检查器**（Typechecker）。通过类型检查器检查的程序称为**类型良好**（Well-typed）的。
...
一个具有良好行为的程序必然是安全的。类型系统的主要目标是通过排除所有未捕获错误来确保语言安全。不过大多数类型系统通常会确保良好行为从而隐含的保证类型安全。

静态检查语言并不保证绝对安全，因为一些语言禁止的错误集合不包括所有的未捕获错误，称为**弱检查**（Weakly checked）。
...
-   **执行效率**：在 ML 中，准确的类型信息可消除对指针解引用时的 nil 检查，避免了运行时的昂贵测试。
-   **开发效率**：类型检查能在编译时捕捉大部分常见错误，使调试过程更简单；程序员可以利用类型检查器来验证代码变更（如当字段的不变性发生变化时，通过更改字段名称来捕捉遗漏的使用）。
-   **编译与模块化**：类型信息构成模块接口，使各模块可以独立编译，从而提高大型系统的编译效率和可维护性。
-   **大规模协作**：明确的类型接口降低了各模块间的依赖，便于团队协作和局部代码重构。
-   **安全保障**：防止诸如将整数随意转换为指针（或对象）类型的危险操作，避免可能导致系统完全被攻破的严重安全漏洞。
...
-   **透明性**（Transparent）：程序员能够直观预见程序是否能通过类型检查，且错误原因应明确。
-   **可执行性**（Enforceable）：类型声明应尽可能在静态阶段得到验证，必要时辅以动态检查，保证声明与程序行为的一致。

形式化类型系统通常包括以下步骤：

1.  **描述语法**（Syntax）：定义类型和项（Term）的结构。
2.  **确定作用域规则**（Scoping rules）：通常采用静态作用域，将标识符与其声明位置严格对应。而缺乏静态作用域则称为动态作用域。
3.  **建立类型规则**（Type rules）：描述项 $M$ 与 类型 $A$ 之间的关系。如 has-type 关系 $M : A$、子类型关系 $A <: B$，以及类型等价关系 $A = B$。
4.  **引入静态类型环境**（Static typing environments）：记录程序中自由变量的类型，对应于编译器在类型检查阶段使用的符号表。如 has-type 关系 $M : A$ 通常写作为 $\Gamma\vdash M : A$，表示在环境 $\Gamma$ 下，$M$ 的类型为 $A$。
...
类型系统规定了编程语言中的类型规则，与具体的类型检查算法（Typechecking algorithm）相互独立，类似于用形式文法（Formal grammar）描述语法而不涉及具体解析算法（Parsing algorithm）。
...
-   $true$ 的类型为 $Bool$：$\empty\vdash true : Bool$
-   $x+1$ 的类型为 $Nat$，前提是 $x$ 的类型为 $Nat$：$\empty, x : Nat\vdash x+1 : Nat$
...
-   加法规则：规定如果 $M$ 和 $N$ 均表示自然数，则 $M+N$ 表示自然数，且环境 $\Gamma$ 对 $M$、$N$ 和 $M+N$ 一致传递
...
则 $1+true$ 可被类型化，这表明类型推断问题对具体类型系统非常敏感，算法的难易取决于类型系统的设计。

### 实践中的类型推断

-   对于显式类型化的语言（如 Pascal），类型推断较为容易
...
-   **操作语义**（Operational semantics）：若 $\empty\vdash M:A$，且 $M$ 归约为 $M’$，则有 $\empty\vdash M’: A$

在这两种情况下，类型健全性定理均断言：良好类型化的程序在计算时不会产生执行错误，连接了类型系统与程序语义之间的关系。
