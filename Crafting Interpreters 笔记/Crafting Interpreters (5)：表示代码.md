...
与其繁琐地手写每个类的定义、字段声明、构造函数和初始化器，不如编写一个用于生成这些样板代码的生成器脚本。创建一个 `generate_ast.rb` 文件：

```ruby
class GenerateAst
  def initialize(output_dir, basename, productions)
  ...
  def make_ast
    path = File.join(@output_dir, "#{@basename.downcase}.rb")
    File.open(path, 'w:UTF-8') do |file|
      file.puts '# frozen_string_literal: true'
      file.puts
      file.puts '# This file was generated by GenerateAst. Do not edit it directly.'
      file.puts
      file.puts "class #{@basename}; end"
...
output_dir = File.join(script_dir, 'lib', 'parser')

GenerateAst.new(output_dir, 'Expression', [
                  'Literal : value',
...
之后就会在 `lib/parser/expression.rb` 下生成如下代码：
...
可以将表达式访问者代码编入到 `generate_ast.rb` 脚本中用来自动生成。
...
def make_production(file, head, names)
  ...
  file.puts
  file.puts '  def accept(visitor)'
  file.puts "    visitor.visit_#{head.downcase}(self)"
  file.puts '  end'
  file.puts 'end'
end
...
def make_visitor(file)
  ...
  @productions.each do |production|
    ...
    file.puts "  def visit_#{head.downcase}(#{head.downcase})"
    file.puts '    raise NotImplementedError,'
    file.puts '          "[#{__FILE__}:#{__LINE__}]: `#{self.class}##{__method__}` must be implemented."' # rubocop:disable Lint/InterpolationCheck
    file.puts '  end'
  end
  file.puts 'end'
end
...
```ruby
expr = Binary.new(
    Unary.new(Token.new(:MINUS, '-', nil, 1), Literal.new(123)),
    Token.new(:STAR, '*', nil, 1),
    Group.new(Literal.new(45.67))
  )
